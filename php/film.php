  ...
  <div class="container col-12">
  	...
  	<?php
  	 	if(isset($_GET["film"])) {
			...
			if(isset($_GET["s"]) && $_GET["s"]==1) {
		  	$s = 1;
				$sql = "select * from serie  where id = $idfilm";
  	 	} else {
		   	$s = 0;
      ...
      exit;
  	  ...
  	  while ($ligne){
  		...
  		while ($ligne) {
				...
				$resume = $ligne["resume"];
				$date = $ligne["date_sortie"];
				$duree = $ligne["nb_episodes"];
  	 		echo "<h3 class=\"t3 text-uppercase text-center\">$titre</h3>";
				echo "<h4><strong>Résumé</strong></h4>";
				echo "<p>$resume</p>";
				echo "<h4><strong>Informations</strong></h4>";
				echo "<p><strong>Date de sortie :</strong> $date</p>"; 
				echo "<p><strong>Durée :</strong> $duree épisodes</p>";
  	    $ligne=pg_fetch_array($resultat);
  	 	}
    }
		else {
      ...
      ?>
			...
			<?php
				...
				$sql="select f.id, f.titre from film f, serie s, episode e where f.id = e.film and s.id = e.serie and e.serie = $idfilm";
				$resultat=pg_query($connect,$sql);
				...
				if (!$resultat){
					...
					echo "<h5><strong>Épisodes</strong></h5>";
					echo "<table class=\"table table-bordered\"><tbody>";
					$ligne=pg_fetch_array($resultat);
					while ($ligne){	
						$titre = $ligne["titre"];
						$idfilm = $ligne["id"];
						echo "<tr><td><a href='episode.php?film=$idfilm'>&#8594; $titre</a></td></tr>";
						$ligne=pg_fetch_array($resultat);
					}
					echo "</tbody></table>";
				}
